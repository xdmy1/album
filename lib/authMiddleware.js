...
// Middleware wrapper for API routes. Top-level try/catch ensures the
// client always sees JSON, never an HTML error page.
...
// Require admin token. Wraps the handler in a top-level try/catch so that
// ANY uncaught error (including from third-party imports or module-load
// glitches in dev) becomes a JSON 500 response instead of Next.js's HTML
// error page — which on the client side surfaces as the cryptic
...
export function requireAuthOrAdmin(handler, { editorOnlyForFamily = false } = {}) {
  return async (req, res) => {
    try {
      ...
      console.error('[requireAuthOrAdmin] unhandled error:', err)
      if (!res.headersSent) {
        return res.status(500).json({
          error: err?.message || 'Eroare internă neașteptată',
          where: 'requireAuthOrAdmin',
          stack: process.env.NODE_ENV === 'production' ? undefined : err?.stack,
        })
      }
    }
