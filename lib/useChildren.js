...
import { authenticatedFetch } from './pinAuth'
...
export function useChildren(familyId) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true)
        const sRes = await authenticatedFetch(`/api/album-settings/get?familyId=${familyId}`)
        const sJson = await sRes.json()
        if (cancelled) return

        if (sRes.ok && sJson.settings?.is_multi_child) {
          setIsMultiChild(true)
          const cRes = await authenticatedFetch(`/api/children/list?familyId=${familyId}`)
