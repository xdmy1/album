...
export const parseAdminResponse = async (response) => {
  ...
  // Next.js dev's overlay HTML includes the actual error message in a
  // <pre> or similar element — try to surface it so the user doesn't have
  // to switch to the dev tools tab.
  const text = await response.text().catch(() => '')
  const looksLikeHtml = /^\s*<!?(doctype|html)/i.test(text)

  // Try to extract a Next.js error message embedded in the HTML page.
  const extracted =
    /"message":"((?:[^"\\]|\\.)*)"/.exec(text)?.[1] ||
    />Error:\s*([^<]+)</i.exec(text)?.[1] ||
    null
  ...
  const hint = looksLikeHtml
    ? (status === 404
        ...
        : decoded
          ? `Server crash (status ${status}): ${decoded}`
          : `Serverul a returnat HTML în loc de JSON (status ${status}). Verifică terminalul unde rulează 'next dev' pentru eroarea reală.`)
