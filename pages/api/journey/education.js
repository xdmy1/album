// Educational progress narrative — GET (read) + POST (upsert). Family tier.
...
async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const row = await getChildRecord('education_notes', familyId, childId)
      return res.status(200).json({ success: true, education: row })
    }
    if (req.method === 'POST') {
      if (!canWrite(req)) return res.status(403).json({ error: 'Necesită rol de editor' })
      const childId = req.body?.childId || null
      const body = typeof req.body?.body === 'string' ? req.body.body : ''
      const row = await upsertChildRecord('education_notes', familyId, childId, { body })
      return res.status(200).json({ success: true, education: row })
    }
...
export default requireAuthOrAdmin(requireFeature('educationalProgress', handler))
