...
export default async function handler(req, res) {
  ...
  if (!familyId || !year) return res.status(400).json({ error: 'familyId and year required' })

  const yearInt = parseInt(year)
  const startDate = new Date(yearInt, 0, 1).toISOString()
  const endDate = new Date(yearInt + 1, 0, 1).toISOString()
  ...
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="${family?.name || 'family'}-${year}.json"`)
