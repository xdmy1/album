  ...
  const createTable = async () => {
    ...
    try {
      ...
      if (familiesError || !families || families.length === 0) {
        setStatus('❌ Error: No families found in database')
        setIsLoading(false)
        return
      }
      ...
      if (testError) {
        if (testError.message.includes('does not exist') || 
          ...
          setStatus(`❌ Table does not exist. Error: ${testError.message}`)
        } else {
      ...
      if (setupResult.ok) {
        setStatus(`✅ Success! Categories setup completed for ${setupData.familiesSetup}/${setupData.totalFamilies} families`)
      } else {
        setStatus(`❌ Setup failed: ${setupData.error}`)
      }
  ...
  return (
    <div style={{ 
      ...
      <div style={{ 
        ...
        <h3>🔧 Manual Table Creation Required</h3>
        <p>Go to your <strong>Supabase Dashboard → SQL Editor</strong> and run this SQL:</p>
        <pre style={{ 
...
ALTER TABLE family_categories ENABLE ROW LEVEL SECURITY;`}
      ...
      {status && (
        <div style={{ 
          ...
          border: `1px solid ${status.startsWith('✅') ? '#22c55e' : '#ef4444'}`,
          borderRadius: '8px',
          marginTop: '20px',
          whiteSpace: 'pre-wrap'
        }}>
